...
const optCoerceEmail = z.preprocess(
    v => (v === null || v === '' ? undefined : v),
    z.coerce.string().trim().email().optional()
);
...
const reqDetalleMin5 = z.preprocess(
    v => (v == null ? '' : v),
    z.coerce.string().min(5)
);

const optCoerceStrMin2 = z.preprocess(
    v => (v === null || v === '' ? undefined : v),
    z.coerce.string().trim().min(2).optional()
);

export const publicCreateSchema = z.object({
    socioNumero: optCoerceStr,
    dni: optCoerceStr,
    nombre: reqStrMin2,
    apellido: optCoerceStrMin2,
    telefono: optCoerceStr,
    email: optCoerceEmail,
    categoria: z.preprocess(
        v => (v == null ? undefined : v),
        z.enum(['reclamo','consulta','recomendacion','otro'])
    ),
    detalle: reqDetalleMin5
});

export const listSchema = z.object({
    status: z.enum(['pending','resolved']).optional(),
    categoria: z.enum(['reclamo','consulta','recomendacion','otro']).optional(),
...
export async function updateStatus(req, res, next) {
    try {
        ...
        if(!row) return res.status(404).json({ error: { code: 'not_found', message: 'Contacto no encontrado' }});
        res.json({ ok: true })
    } catch (e) {
        next(e);
    }
